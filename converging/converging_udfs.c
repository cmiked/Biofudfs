...
DEFINE_PROFILE(inlet_pressure_pat_a,t,i)
{
  #if !RP_HOST
  face_t f;
  #endif /* !RP_HOST */
...
DEFINE_PROFILE(inlet_pressure_pat_b,t,i)
{
  #if !RP_HOST
  face_t f;
  #endif /* !RP_HOST */
...
DEFINE_PROFILE(outlet_varying_flux,thread,i)
{
 face_t face;
 #if !RP_HOST /* SERIAL or NODE */
    ...
    begin_f_loop(face,thread)
      ...
      # endif
      {
        F_PROFILE(face,thread,i) = 2*66.94e-4 * (exp(-4.0*CURRENT_TIME) - exp(-10.0*CURRENT_TIME) );
